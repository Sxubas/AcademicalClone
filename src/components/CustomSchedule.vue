  ...
  <div class="container">
    <div id="background-grid" class="schedule-grid">
      <div
        ...
        :key="n"
        :class="['cell']"
      >{{ textBackgroundCell(calcPositionCell(n)) }}</div>
    </div>
...
export default {
  ...
  computed: {
    ...
    totalCells() {
      return (
        (this.timeGround[1] - this.timeGround[0]) *
        2 *
        (this.weekGround.length + 1)
      );
    },
    scheduleArray() {
  ...
  methods: {
    colClass(day) {
      let x =
        "calc(" +
        100 / this.weekGround.length +
        "% - " +
        150 / this.weekGround.length +
        "px)";
      return { width: x };
    },
    ...
    coursePosition(course) {
      if (course) {
        ...
        try{
            let hash = 0;
        for (let i = 0; i < course.nrc.length; i++) {
            hash = course.nrc.charCodeAt(i) + ((hash << 5) - hash);
        }
        ...
        //console.log(duracion);
        return { "grid-column": col, "grid-row": row + " / span " + duracion ,"background-color":"#"+colorHex };
      } else return {};
    },
    hoveredStyle(hoveredClass) {
      if (this.hoveredClassSchedules.conflicts.length > 0) {
        return {
          "background-color": "rgba(212,98,98,0.7)",
    ...
    classTitle(title) {
      if (title.length > 35) return title.substring(0, 38) + "...";
      else return title;
    }
  },
  mounted() {
    ...
    grid.style.gridTemplateColumns =
      "100px repeat(" + this.weekGround.length + ",1fr)";
